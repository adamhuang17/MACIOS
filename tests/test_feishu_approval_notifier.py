...
@pytest.mark.asyncio
async def test_feishu_approval_notifier_uses_private_delivery_target() -> None:
    store = InMemoryEventStore()
    repo = PilotRepository(store)
    client = FakeFeishuClient()
    notifier = FeishuApprovalNotifier(repo, client)
    ...
    assert client.sent_cards[0]["receive_id"] == "ou_user"
    assert client.sent_cards[0]["receive_id_type"] == "open_id"
