...
从 agent_hub.core.tracer 迁移至此；core.tracer 保留为 re-export 向后兼容层。
...
Example::
    ...
    from agent_hub.runtime.observability.tracing import SpanContext
...
class SpanContext:
    ...
    def __exit__(
        ...
        # 写入全局 TraceStore（向后兼容：core.trace_store 在迁移完成前保留引用）
        try:
